...
RUN apt update && apt upgrade -y && apt install -y $BUILD_LIBS && docker-php-ext-configure gd --with-jpeg --with-freetype && docker-php-ext-install -j$(nproc) pdo_pgsql opcache intl gd && pecl install apcu-5.1.23 && docker-php-ext-enable apcu && pecl install redis-6.1.0 && docker-php-ext-enable redis && apt purge -y $BUILD_LIBS && rm -rf /var/lib/apt/lists/*
...
ENTRYPOINT rm -rf var/cache && php bin/console c:c && php bin/console doctrine:migrations:migrate --no-interaction && mkdir -p var/cache/prod/starxen/file_manager/files && chmod 777 var/ -R && apache2-foreground